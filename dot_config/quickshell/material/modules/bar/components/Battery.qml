...
Item {
    ...
    readonly property real componentScale: QsConfig.Config.barComponents.scale.battery

    implicitWidth: batteryContainer.width * componentScale
    implicitHeight: 24 * componentScale
    ...
    readonly property color normalColor: {
        if (isCritical) return barColors.error
        if (isLow) return barColors.warning
        if (batteryLevel >= 60) return barColors.textDim
        return Qt.rgba(barColors.foreground.r, barColors.foreground.g, barColors.foreground.b, 0.6)
    }
